...
def house_predictor(all_datadf):
    ...
    besthousedf=all_datadf[['price', 'stateOfBuilding', 'epcScore', 'postalCode', 'province', 'typeOfProperty', 'numberOfBedrooms', 'livableArea', 'surfaceAreaOfPlot']]




    #split the data into train and test
    X_train, X_test, y_train, y_test = train_test_split(besthousedf.drop('price', 1),
                                                        besthousedf['price'],
    ...
    house_fit_forest_pipe = forest_pipe.fit(X_train, y_train)
    ...
    joblib.dump(house_fit_forest_pipe , 'house_pred_model.pkl')


house_predictor(all_datadf)